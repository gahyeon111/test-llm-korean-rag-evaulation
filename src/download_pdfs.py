...
두 가지 경로:
  --from-hf   HF 데이터셋 repo 안에 PDF 가 들어있으면 거기서 직접 받는다 (권장, URL 만료 없음)
  (기본)      documents.csv 의 원문 URL 로 받는다
...
# 일부 기관 사이트는 기본 UA 를 막고 안내 HTML 을 돌려준다.
HEADERS = {
...
def nfc(text: str) -> str:
    """macOS/Linux 간 한글 자모 분리(NFD) 차이를 흡수한다."""
...
def download_one(url: str, dest: Path, timeout: int, retries: int,
                 html_budget: list[int]) -> tuple[str, dict]:
...
def main() -> int:
    ...
    for i, doc in enumerate(docs.itertuples(index=False), start=1):
        ...
        if dest.exists() and dest.stat().st_size > 0 and dest.read_bytes()[:4] == b"%PDF":
            ...
            status, meta = download_one(url, dest, args.timeout, args.retries, html_budget)
